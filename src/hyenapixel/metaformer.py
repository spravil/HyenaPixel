...
# Modified version of original implementation (https://github.com/sail-sg/metaformer) of "MetaFormer Baselines for Vision" (https://arxiv.org/abs/2210.13452).
...
class Attention(nn.Module):
    ...
    def __init__(
        self, dim, head_dim=32, num_heads=None, qkv_bias=False, attn_drop=0.0, proj_drop=0.0, proj_bias=False, **kwargs
    ):
        super().__init__()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dwconv = nn.Conv2d(
            med_channels, med_channels, kernel_size=kernel_size, padding=padding, groups=med_channels, bias=bias
        )  # depthwise conv
...
class MetaFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.multi_token_mixer = False
        if isinstance(token_mixer, list):
            self.multi_token_mixer = True
            self.norm1 = nn.ModuleList(norm_layer(dim) for _ in range(len(token_mixer)))
            self.token_mixer = nn.ModuleList(token_mixer[i](dim, drop=drop) for i in range(len(token_mixer)))
            self.drop_path1 = nn.ModuleList(
                DropPath(drop_path) if drop_path > 0.0 else nn.Identity() for _ in range(len(token_mixer))
            )
            self.layer_scale1 = nn.ModuleList(
                Scale(dim=dim, init_value=layer_scale_init_value) if layer_scale_init_value else nn.Identity()
                for _ in range(len(token_mixer))
            )
        else:
    ...
    def forward(self, x):
        if self.multi_token_mixer:
            y = self.res_scale1(x)
            for layer_scale1, drop_path1, token_mixer, norm1 in zip(
                self.layer_scale1, self.drop_path1, self.token_mixer, self.norm1
            ):
                y = y + layer_scale1(drop_path1(token_mixer(norm1(x))))
            x = y
        else:
...
class MetaFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.features_only:
            self.fnorm = nn.ModuleList([nn.LayerNorm(dims[i]) for i in range(num_stage)])
        ...
        if self.features_only:
            ...
            for i in range(self.num_stage):
                ...
                if i in self.out_indices:
                    out.append(self.fnorm[i](x).permute(0, 3, 1, 2).contiguous())
            return out
        else:
            for i in range(self.num_stage):
                x = self.downsample_layers[i](x)
...
class MetaFormerNoPool(MetaFormer):
    def __init__(
        self,
        in_chans=3,
        num_classes=1000,
        ...
        token_mixers=nn.Identity,
        mlps=Mlp,
        norm_layers=partial(LayerNormWithoutBias, eps=0.000001),
        drop_path_rate=0,
        head_dropout=0,
        drop_rate=0.0,
        layer_scale_init_values=None,
        res_scale_init_values=[None, None, 1, 1],
        output_norm=partial(nn.LayerNorm, eps=0.000001),
        head_fn=nn.Linear,
        **kwargs,
    ):
        super().__init__(
